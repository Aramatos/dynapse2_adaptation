...
def TCU_single(board, profile_path, number_of_chips):
    #||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
    #Auto_Save_set_up
    ...
    if neuron_config['sweep']==True:
        ...
        spike_times=spike_time_arrays(output_events,nvn,pvn,pcn,sstn)
        plot_psth(test_config,spike_times[3], bin_size=0.031)
        np.save("output_events",output_events)
        np.save(test_config['dir_path']+"/sst_"+test_config['dir_path'],  output_events)
    #||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
